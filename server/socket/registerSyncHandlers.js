function registerSyncHandlers({ socket, EVT, log, relayToRoomPeers }) {
    socket.on(EVT.REQUEST_SYNC, () => {
        log(socket.id, '请求全量同步');
        relayToRoomPeers(socket, EVT.REQUEST_SYNC);
    });
    ...
    socket.on(EVT.FULL_STATE_SYNC, (data) => {
        ...
        relayToRoomPeers(socket, EVT.FULL_STATE_SYNC, data);
    });

    socket.on(EVT.SYNC_RESET, () => {
        log(socket.id, '重置游戏');
        relayToRoomPeers(socket, EVT.SYNC_RESET);
    });
}
