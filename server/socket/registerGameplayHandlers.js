...
function registerGameplayHandlers({ socket, EVT, log }) {
    socket.on(EVT.PLAY_TO_FIELD, (data) => socket.broadcast.emit(EVT.OPPONENT_PLAYED_TO_FIELD, data));
    socket.on(EVT.PLAY_TO_BOND, (card) => socket.broadcast.emit(EVT.OPPONENT_PLAYED_TO_BOND, card));
    socket.on(EVT.RETURN_TO_HAND, (card) => socket.broadcast.emit(EVT.OPPONENT_RETURNED_CARD, card));

    socket.on(EVT.PLAYER_DRAW, (data) => {
        log(socket.id, `抽牌 → ${cardName(data?.card)}`);
        socket.broadcast.emit(EVT.OPPONENT_DRAW_CARD, data);
    });
